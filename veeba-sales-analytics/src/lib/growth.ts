...
export function calculateGrowth(current: number, previous: number): GrowthResult {
  ...
  if (previous === 0 && current > 0) {
    return { direction: 'new', percentLabel: '', changeValue }
  }
  if (previous === 0 && current === 0) {
    return { direction: 'flat', percentLabel: '0.0%', changeValue: 0 }
  }

  const percent = (changeValue / previous) * 100

  if (!Number.isFinite(percent)) {
    return { direction: 'flat', percentLabel: '0.0%', changeValue }
  }

  const rounded = Math.abs(percent).toFixed(1)

  if (percent > 0) {
    return { direction: 'up', percentLabel: `+${rounded}%`, changeValue }
  }
  if (percent < 0) {
    return { direction: 'down', percentLabel: `-${rounded}%`, changeValue }
  }
  return { direction: 'flat', percentLabel: '0.0%', changeValue }
}
