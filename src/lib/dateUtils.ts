...
/** Returns the most recent fully-completed month as an <input type="month"> value. */
export function getLastCompletedMonthValue(): string {
  const now = new Date()
  ...
  return toInputMonthValue(month, year)
}

/**
 * Given an "as of" month value, returns the start of that month's financial
 * year (April) as an <input type="month"> value. Veeba's FY runs Apr–Mar.
 */
export function getFYStartValue(asOfValue: string): string {
  const parsed = parseInputMonthValue(asOfValue)
  if (!parsed) return asOfValue
  const fyStartYear = parsed.month >= 3 ? parsed.year : parsed.year - 1
  return toInputMonthValue(3, fyStartYear) // April = index 3
}
