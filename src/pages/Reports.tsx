...
export default function Reports() {
  const { currentRows, previousRows, hasReport, appliedCurrentRange, appliedPreviousRange } = useReport()
      ...
      {!hasReport ? (
        ...
        <>
          <p className="reports-context">
            Using data from: {appliedCurrentRange.startValue} to {appliedCurrentRange.endValue}, plus the
            comparison period ({appliedPreviousRange.startValue} to {appliedPreviousRange.endValue}) —
            both are available as Columns so you can pivot across years.
          </p>
          <PivotBuilder currentRows={combinedRows} />
        </>
      )}
    </div>
  )
